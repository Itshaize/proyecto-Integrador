...
try {
  ...
  const adminId = Number(
    (
      db
        ...
        .get(ADMIN.correo) as { id_usuario: number }
    ).id_usuario,
  );
  ...
  const adultUserId = Number(
    (
      db
        ...
        .get(ADULT.correo) as { id_usuario: number }
    ).id_usuario,
  );
  ...
  const adultId = Number(
    (
      db
        .prepare("SELECT id_adulto FROM adultos_mayores WHERE id_usuario=?")
        .get(adultUserId) as { id_adulto: number }
    ).id_adulto,
  );
  ...
  const latestLocation = db
    ...
    .get(adultId) as { id_ubicacion: number } | undefined;
  if (latestLocation) {
    db.prepare(
  ...
  const hasDemoAlert = db
    .prepare("SELECT 1 FROM alertas WHERE id_adulto=? AND tipo='SOS' LIMIT 1")
    .get(adultId);
  if (!hasDemoAlert) {
    db.prepare(
      `INSERT INTO alertas
